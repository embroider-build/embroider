...
6. Grab the [example babel config](https://github.com/embroider-build/addon-blueprint/blob/main/files/__addonLocation__/babel.config.json) and save it as `addon/babel.config.json`
   - If you addon requires template transforms in order to publish to a shareable format. Apply transforms using the `@embroider/addon-dev/template-transform-plugin`. View how to use this in the [example babel.config.js](https://github.com/embroider-build/addon-blueprint/blob/main/files/__addonLocation__/babel.config.json)
7. Grab the [example rollup config](https://github.com/embroider-build/addon-blueprint/blob/main/files/__addonLocation__/rollup.config.js) and save it as `addon/rollup.config.js`.
