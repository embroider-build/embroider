...
Another way to provide the consuming app with static assets from your addon is a push-based approach very similar to the `/public` folder of v1 addons. While the pull-based approach above is preferable in general, for example in cases where you do not need to refer to the asset directly, but want to have a predetermined URL, you can choose te push-based way instead.
...
If you have many files you want to expose this way, you can also add the `addon.publicAssets()` plugin from `@embroider/addon-dev` to your Rollup config to automate the generation of this mapping data.
...
module: {
  rules: [
    {
      test: /\.ya?ml$/i, // make this match to what files you want to get imported through your loader
