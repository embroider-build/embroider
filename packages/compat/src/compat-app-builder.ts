...
import { outputJSONSync, readJSONSync, rmSync, writeFileSync, realpathSync, readFileSync } from 'fs-extra';
...
export class CompatAppBuilder {
  ...
  async build(inputPaths: OutputPaths<TreeNames>) {
    ...
    this.addHTMLEntrypoints(inputPaths.htmlTree);
  ...
  private addHTMLEntrypoints(htmlTreePath: string) {
    const appPath = join(locateEmbroiderWorkingDir(this.compatApp.root), 'rewritten-app');

    // write all files from the html tree to the output
    const htmlFiles = walkSync(htmlTreePath, {
      directories: false,
    });

    for (let file of htmlFiles) {
      writeFileSync(join(appPath, file), readFileSync(join(htmlTreePath, file)));
    }
  }
