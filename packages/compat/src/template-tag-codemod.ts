import { prebuild } from './default-pipeline';
...
import { statSync, readdirSync, readFileSync, writeFileSync, type PathLike } from 'fs';
  ...
  async build() {
    ...
    for await (const current_file of walkSync(tmp_path)) {
      if (hbs_file_test.test(current_file) && this.options.shouldTransformPath(current_file)) {
                  ...
                  async ImportDeclaration(import_declaration: NodePath<t.ImportDeclaration>) {
                    ...
                    if (extractor) {
                      ...
                      if (result.type === 'real') {
                        ...
                        if (owner_package) {
                          const t = resolver.reverseSearchAppTree(owner_package, result.filename);
                          if (t || owner_package.isV2App()) {
                            let relative_import_path = explicitRelative(owner_package.root, result.filename);
                            const ext_name = externalName(owner_package.packageJSON, relative_import_path) || '';

                            // change import path to real one
                            import_declaration.node.source.value = ext_name;
                            import_bucket.push(import_declaration);
                          } else {
                            console.log('cannot locate package for: ', result.filename);
                          }
                        } else {
                          console.log('cannot locate package for: ', result.filename);
                        }
        ...
        const backing_class_filename: string =
          'filename' in backing_class_resolution ? (backing_class_resolution.filename as string) : '';
        ...
        const backing_class_src = readFileSync(backing_class_filename as PathLike).toString();
        ...
        const app_relative_path = join('app', explicitRelative(tmp_path, current_file));
        ...
        if (!is_template_only) {
          ...
          const app_relative_path = join('app', explicitRelative(tmp_path, backing_class_filename));
