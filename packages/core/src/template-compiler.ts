...
function getEmberExports(templateCompilerPath: string): EmbersExports {
  ...
  console.log(`PATCH check ${templateCompilerPath}=${needsPatch}`);
  ...
  require('fs').writeFileSync('/tmp/before.js', source, 'utf8');
  ...
  require('fs').writeFileSync('/tmp/after.js', source, 'utf8');
