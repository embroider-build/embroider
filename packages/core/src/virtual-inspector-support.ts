...
export interface VirtualInspectorSupportResponse {
  type: 'inspector-support';
  specifier: string;
}

export function renderInspectorSupport(
  response: VirtualInspectorSupportResponse,
  resolver: Resolver
  ...
  return { src: getInspectorSupport(), watches: [] };
}
