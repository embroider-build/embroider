...
export default function getConfig(
  ...
  if (own) {
    if (path.node.arguments.length !== 0) {
      throw error(path as NodePath, `getOwnConfig takes zero arguments, you passed ${path.node.arguments.length}`);
    }
    packageName = undefined;
  } else {
    if (path.node.arguments.length !== 1) {
      throw error(path as NodePath, `getConfig takes exactly one argument, you passed ${path.node.arguments.length}`);
    }
    let packageNode = path.node.arguments[0];
    if (packageNode.type !== 'StringLiteral') {
      let assertResult = assertArray(path.get('arguments'))[0] as NodePath;
      throw error(assertResult, `the argument to getConfig must be a string literal`);
    }
    packageName = packageNode.value;
  }
  let config: unknown | undefined;
  let pkg = targetPackage(sourceFile(path as NodePath, state), packageName, packageCache);
  if (state.opts.mode === 'compile-time') {
    if (pkg) {
      config = state.opts.userConfigs[pkg.root];
    }
    let collapsed = collapse(path as NodePath<Expression>, config);
    ...
    path.replaceWith(
      callExpression(memberExpression(path.get('callee').node as Expression, identifier('_runtimeGet')), [pkgRoot])
    );
  }
}
...
function collapse(path: NodePath<Expression>, config: any) {
  while (true) {
    ...
    let object = parentPath.get('object') as NodePath;
    if (parentPath.isMemberExpression() && object.node === path.node) {
      ...
      if (parentPath.node.computed) {
        ...
        if (evalProperty.confident) {
          ...
          path = parentPath as NodePath<Expression>;
          continue;
        }
      } else {
      ...
      if (castParentPath.get('object').node === path.node) {
        ...
        if (castParentPath.node.computed) {
          ...
          if (evalProperty.confident) {
            ...
            path = castParentPath as NodePath<Expression>;
            continue;
          }
        } else {
          if (property.isIdentifier()) {
