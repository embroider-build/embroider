...
Qmodule('fastboot basics', function (hooks) {
  ...
  test('the macro systems sees the right environment', async function (assert) {
    assert.equal(
      doc.querySelector('[data-test="macro-example"]').textContent.trim(),
      'macro-example: I am in fastboot'
    );
  });
});
